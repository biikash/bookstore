<?php
$conn=mysqli_connect("localhost", "user", "Bikash@123","ecom");
...
function cart(){
    if(isset($_GET['add_cart']))
    {
        ...
        $check_product="SELECT `bookid`, `ip_add`, `quantity` FROM `ecom`.`cart` WHERE  ip_add='$ip' AND bookid='$book_id'";
        ...
        if(mysqli_num_rows($run_check)>0)
        {
          ...
          $insert_cart="INSERT INTO `ecom`.`cart`(`bookid`, `ip_add`) VALUES ('$book_id','$ip')";
...
function total_items(){
    ...
    if(isset($_GET['add_cart']))
    {
        $get_items="SELECT * FROM `ecom`.`cart` WHERE `ip_add`='$ip'";
...
function mycart() {
  ...
  $ip = getIpAdd();
    $count = 1;
  $get_cart = "SELECT * FROM `ecom`.`Books` WHERE `book_id` IN (SELECT `bookid` FROM cart WHERE ip_add LIKE '::1')";
...
function getcats(){
	...
	$query4="SELECT * from `ecom`.`category`";
...
function getauths(){
	...
	$query3="SELECT DISTINCT author FROM `ecom`.`Books`";
...
function get_bycat(){
  ...
  if(isset($_GET['category'])){
    ...
    $get_cat_pro = "SELECT * FROM `ecom`.`Books` WHERE category LIKE '$cat_id'";
